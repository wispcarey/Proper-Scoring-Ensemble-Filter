...
from psef.utils.legacy import L63, L96, rk4, DoublingMap1D
from psef.utils.legacy import AverageMeter, mystery_operator, get_mean_std, get_test_noise_generator
from psef.utils.legacy import post_process, mean0
from psef.plotting.visualization import (
...
from psef.filters.localization import dist2coeff, create_loc_mat, pairwise_distances
from psef.losses.scoring_rules import (
    compute_loss,
    compute_es,
...
from psef.models.architectures import (
    NaiveNetwork,
    SetTransformer,
    Simple_MLP,
...
from psef.filters.benchmark_analysis import ensemble_kalman_filter_analysis, bootstrap_particle_filter_analysis
...
def _build_pf_cache_filename(
    ...
    return f"{base}_{seed_value}{obs_suffix}.pt"
...
def _load_cached_pf_avg_results(loader, args):
    """
    Load PF cache used for verification.

    - If args.pf_verification_seed is None: load the averaged PF cache.
    - Otherwise: load the per-seed PF cache for that seed.
    """
    ...
    pf_verification_seed = getattr(args, "pf_verification_seed", 42)
    use_avg_cache = pf_verification_seed is None
    cache_filename = _build_pf_cache_filename(
        args,
        batch_size=batch_size,
        traj_len=traj_len,
        avg=use_avg_cache,
        seed=pf_verification_seed,
    )
    cache_filepath = os.path.join(cache_dir, cache_filename)

    if not os.path.exists(cache_filepath):
        cache_desc = "averaged" if use_avg_cache else f"seed={pf_verification_seed}"
        raise FileNotFoundError(
            f"Required PF verification cache ({cache_desc}) not found: {cache_filepath}. "
    ...
    print(f"Loading cached PF results ({'avg' if use_avg_cache else f'seed={pf_verification_seed}'}) from: {cache_filepath}")
...
def _get_pf_cached_mode_means(entry: Dict[str, Any], mode: str) -> Optional[torch.Tensor]:
    mode_key = str(mode).lower()
    if mode_key == "post":
        return _get_pf_cached_post_means(entry)
    if mode_key == "prior":
        return entry.get("prior_means", None)
    return entry.get(f"{mode_key}_means", None)


def _get_pf_cached_mode_covs(entry: Dict[str, Any], mode: str) -> Optional[torch.Tensor]:
    mode_key = str(mode).lower()
    if mode_key == "post":
        return _get_pf_cached_post_covs(entry)
    if mode_key == "prior":
        return entry.get("prior_covs", None)
    return entry.get(f"{mode_key}_covs", None)


def _get_pf_mode_pca_directions_from_covs(
    entry: Dict[str, Any],
    mode: str,
    max_dims: int = 3,
) -> Optional[torch.Tensor]:
    """
    Reconstruct PCA directions from cached PF covariances for a given mode.

    Returns:
        torch.Tensor with shape [Tm1, B, D, k], where k=min(max_dims, D),
        or None if the requested covariances are unavailable.
    """
    covs = _get_pf_cached_mode_covs(entry, mode)
    if covs is None or covs.ndim != 4:
        return None
    Tm1, B, D, D2 = covs.shape
    if D != D2 or D <= 0:
        return None

    k = min(int(max_dims), int(D))
    cov_flat = covs.to(torch.float32).reshape(-1, D, D)
...
def _get_pf_post_pca_directions_from_covs(entry: Dict[str, Any], max_dims: int = 3) -> Optional[torch.Tensor]:
    ...
    """
    return _get_pf_mode_pca_directions_from_covs(entry, mode="post", max_dims=max_dims)


def _get_pf_cached_mode_pca_transform(
    entry: Dict[str, Any],
    mode: str,
    range_idx: int,
    bidx: int,
    ...
    """
    Return (mean, basis) for cached PF PCA coordinates at one step/batch item.

    - mean: [D]
    - basis: [D, k], k<=max_dims
    """
    means = _get_pf_cached_mode_means(entry, mode)
    dirs = _get_pf_mode_pca_directions_from_covs(entry, mode, max_dims=max_dims)
    if means is None or dirs is None:
        return None, None
    if means.ndim != 3 or dirs.ndim != 4:
        return None, None
    if not (0 <= range_idx < means.shape[0] and 0 <= bidx < means.shape[1]):
        return None, None
    if not (0 <= range_idx < dirs.shape[0] and 0 <= bidx < dirs.shape[1]):
        return None, None
    return means[range_idx, bidx], dirs[range_idx, bidx]


def _empty_pf_sed_breakdown(
    batch_size: int,
    device: torch.device,
...
def _append_pf_sed_breakdown(all_results: Dict[str, torch.Tensor], pf_sed_breakdown: Dict[str, torch.Tensor]) -> None:
    all_results['pf_sed'] = torch.cat((all_results['pf_sed'], pf_sed_breakdown['overall']))
    all_results['pf_sed_state_mean'] = torch.cat((all_results['pf_sed_state_mean'], pf_sed_breakdown['state_mean']))
    all_results['pf_sed_pca_mean'] = torch.cat((all_results['pf_sed_pca_mean'], pf_sed_breakdown['pca_mean']))
    all_results['pf_sed_state_dims'] = torch.cat((all_results['pf_sed_state_dims'], pf_sed_breakdown['state_dims']), dim=0)
    all_results['pf_sed_pca_dims'] = torch.cat((all_results['pf_sed_pca_dims'], pf_sed_breakdown['pca_dims']), dim=0)
...
def _safe_relative_ratio(numerator: torch.Tensor, denominator: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    denom_safe = torch.clamp(denominator, min=eps)
    ratio = numerator / denom_safe
    invalid = (~torch.isfinite(numerator)) | (~torch.isfinite(denominator)) | (denominator <= eps)
    return ratio.masked_fill(invalid, float("nan"))


def _compute_relative_metric_variants(
    step_metric: torch.Tensor,
    step_reference: torch.Tensor,
) -> Tuple[torch.Tensor, torch.Tensor]:
    """
    Return:
    - stepwise relative metric [T, B]: divide per step, preserve time dimension
    - trajwise relative metric [B]: divide trajectory-mean numerator by trajectory-mean denominator
    """
    step_rel = _safe_relative_ratio(step_metric, step_reference)
    traj_metric = torch.mean(step_metric, dim=0)
    traj_reference = torch.mean(step_reference, dim=0)
    traj_rel = _safe_relative_ratio(traj_metric, traj_reference)
    return step_rel, traj_rel


def _append_metric_step_matrix(
    existing_matrix: Optional[torch.Tensor],
    batch_step_values: torch.Tensor,
) -> torch.Tensor:
    """Append [T, B] step metrics as rows [B, T] so they can be masked by trajectory later."""
    if batch_step_values.ndim != 2:
        raise ValueError(f"batch_step_values must be [T,B], got {tuple(batch_step_values.shape)}")
    batch_rows = batch_step_values.transpose(0, 1).to(torch.float32)
    if existing_matrix is None:
        return batch_rows
    if existing_matrix.shape[1] != batch_rows.shape[1]:
        raise ValueError(
            f"Step metric length mismatch: existing T={existing_matrix.shape[1]} vs new T={batch_rows.shape[1]}"
        )
    return torch.cat((existing_matrix, batch_rows), dim=0)


def _get_masked_traj_metric_mean_std(
    ...
    if values is None or not torch.is_tensor(values) or values.numel() == 0:
        return float("nan"), float("nan")
    use_values = values
    if valid_B_mask is not None and values.shape[0] == valid_B_mask.shape[0]:
        use_values = values[valid_B_mask]
    return _get_mean_std_or_nan(use_values)


def _get_masked_step_metric_mean_std(
    step_matrix: Optional[torch.Tensor],
    valid_B_mask: Optional[torch.Tensor],
) -> Tuple[float, float]:
    if step_matrix is None or not torch.is_tensor(step_matrix) or step_matrix.numel() == 0:
        return float("nan"), float("nan")
    use_values = step_matrix
    if valid_B_mask is not None and step_matrix.shape[0] == valid_B_mask.shape[0]:
        use_values = step_matrix[valid_B_mask]
    return _get_mean_std_or_nan(use_values.reshape(-1))


def _add_relative_metric_summary_metrics(
    final_metrics: Dict[str, Any],
    all_results: Dict[str, Any],
    valid_B_mask: Optional[torch.Tensor],
    metric_key: str,
) -> None:
    step_mean, step_std = _get_masked_step_metric_mean_std(all_results.get(f"{metric_key}_step"), valid_B_mask)
    traj_mean, traj_std = _get_masked_traj_metric_mean_std(all_results.get(metric_key), valid_B_mask)

    final_metrics[f"mean_{metric_key}_step"] = step_mean
    final_metrics[f"std_{metric_key}_step"] = step_std
    final_metrics[f"mean_{metric_key}_traj"] = traj_mean
    final_metrics[f"std_{metric_key}_traj"] = traj_std

    # Backward-compatible aliases keep the historical traj-level definition.
    final_metrics[f"mean_{metric_key}"] = traj_mean
    final_metrics[f"std_{metric_key}"] = traj_std


def _add_pf_sed_summary_metrics(
    final_metrics: Dict[str, Any],
    all_results: Dict[str, torch.Tensor],
    valid_B_mask: Optional[torch.Tensor],
) -> None:
    if valid_B_mask is None:
        pf_total = all_results['pf_sed']
        pf_state_mean = all_results['pf_sed_state_mean']
        pf_pca_mean = all_results['pf_sed_pca_mean']
        pf_state_dims = all_results['pf_sed_state_dims']
        pf_pca_dims = all_results['pf_sed_pca_dims']
    else:
        pf_total = all_results['pf_sed'][valid_B_mask]
        pf_state_mean = all_results['pf_sed_state_mean'][valid_B_mask]
        pf_pca_mean = all_results['pf_sed_pca_mean'][valid_B_mask]
        pf_state_dims = all_results['pf_sed_state_dims'][valid_B_mask]
        pf_pca_dims = all_results['pf_sed_pca_dims'][valid_B_mask]

    final_metrics['mean_pf_sed'], final_metrics['std_pf_sed'] = _get_mean_std_or_nan(pf_total)
    final_metrics['mean_pf_sed_state_avg'], final_metrics['std_pf_sed_state_avg'] = _get_mean_std_or_nan(pf_state_mean)
    final_metrics['mean_pf_sed_pca_avg'], final_metrics['std_pf_sed_pca_avg'] = _get_mean_std_or_nan(pf_pca_mean)

    for d_idx in range(PF_CRPS_STATE_DIMS):
        mean_key = f'mean_pf_sed_state_dim{d_idx + 1}'
        std_key = f'std_pf_sed_state_dim{d_idx + 1}'
        final_metrics[mean_key], final_metrics[std_key] = _get_mean_std_or_nan(pf_state_dims[:, d_idx])

    for d_idx in range(PF_CRPS_PCA_DIMS):
        mean_key = f'mean_pf_sed_pca_dim{d_idx + 1}'
        std_key = f'std_pf_sed_pca_dim{d_idx + 1}'
        final_metrics[mean_key], final_metrics[std_key] = _get_mean_std_or_nan(pf_pca_dims[:, d_idx])

    # Backward-compatible aliases for downstream code still expecting CRPS key names.
    final_metrics['mean_pf_crps'] = final_metrics['mean_pf_sed']
    final_metrics['std_pf_crps'] = final_metrics['std_pf_sed']
    final_metrics['mean_pf_crps_state_avg'] = final_metrics['mean_pf_sed_state_avg']
    final_metrics['std_pf_crps_state_avg'] = final_metrics['std_pf_sed_state_avg']
    final_metrics['mean_pf_crps_pca_avg'] = final_metrics['mean_pf_sed_pca_avg']
    final_metrics['std_pf_crps_pca_avg'] = final_metrics['std_pf_sed_pca_avg']
    for d_idx in range(PF_CRPS_STATE_DIMS):
        final_metrics[f'mean_pf_crps_state_dim{d_idx + 1}'] = final_metrics[f'mean_pf_sed_state_dim{d_idx + 1}']
        final_metrics[f'std_pf_crps_state_dim{d_idx + 1}'] = final_metrics[f'std_pf_sed_state_dim{d_idx + 1}']
    for d_idx in range(PF_CRPS_PCA_DIMS):
        final_metrics[f'mean_pf_crps_pca_dim{d_idx + 1}'] = final_metrics[f'mean_pf_sed_pca_dim{d_idx + 1}']
        final_metrics[f'std_pf_crps_pca_dim{d_idx + 1}'] = final_metrics[f'std_pf_sed_pca_dim{d_idx + 1}']


def _compute_dynamic_quantile_sed_from_pf_post(
    ...
    if post_means is not None:
        post_means = torch.as_tensor(post_means, device=ens_post.device, dtype=ens_post.dtype)
    if post_pca_quantiles is not None and pca_dirs is not None and post_means is not None:
        ...
        t_use_pca = min(int(t_use_pca), int(post_means.shape[0]))
        b_use_pca = min(int(b_use_pca), int(post_means.shape[1]))
        if t_use_pca > 0 and b_use_pca > 0:
            ...
            pca_offsets_use = post_means[:t_use_pca, :b_use_pca]
    ...
    try:
        ...
        print(f"Warning: failed to compute quantile SED from PF cache ({exc}). Returning NaN.")
        return out if return_details else out["overall"]
...
def _compute_dynamic_quantile_sed_from_pf_post_step(
    ens_step: torch.Tensor,
    ...
    """Stepwise version of PF-SED used by running test metrics."""
    out = _empty_pf_sed_breakdown(
    ...
    try:
        ...
        print(f"Warning: failed to compute stepwise quantile SED from PF cache ({exc}). Returning NaN.")
        return out if return_details else out["overall"]
...
def _record_running_test_batch_step(
    ...
    ens_step: torch.Tensor,
    true_step: torch.Tensor,
    pf_sed_breakdown: Optional[Dict[str, torch.Tensor]] = None,
) -> None:
    ...
    if not finite_rows.any():
        ...
        if pf_sed_breakdown is not None:
            metric_store["pf_sed"].append(pf_sed_breakdown["overall"].to(torch.float32))
            metric_store["pf_sed_state_mean"].append(pf_sed_breakdown["state_mean"].to(torch.float32))
            metric_store["pf_sed_pca_mean"].append(pf_sed_breakdown["pca_mean"].to(torch.float32))
            metric_store["pf_sed_state_dims"].append(pf_sed_breakdown["state_dims"].to(torch.float32))
            metric_store["pf_sed_pca_dims"].append(pf_sed_breakdown["pca_dims"].to(torch.float32))
        return
...
def _finalize_running_test_batch_metrics(
    ...
    pf_sed_steps = _stack_steps("pf_sed")
    if pf_sed_steps.shape[0] > 0:
        pf_sed_breakdown = _empty_pf_sed_breakdown(
            batch_size=batch_size,
            device=device,
            max_state_dims=PF_CRPS_STATE_DIMS,
            max_pca_dims=PF_CRPS_PCA_DIMS,
        )
        pf_sed_breakdown["overall"] = torch.nanmean(pf_sed_steps, dim=0)
        pf_sed_breakdown["state_mean"] = torch.nanmean(_stack_steps("pf_sed_state_mean"), dim=0)
        pf_sed_breakdown["pca_mean"] = torch.nanmean(_stack_steps("pf_sed_pca_mean"), dim=0)
        pf_sed_breakdown["state_dims"] = torch.nanmean(
            _stack_steps_with_tail("pf_sed_state_dims", (PF_CRPS_STATE_DIMS,)),
            dim=0,
        )
        pf_sed_breakdown["pca_dims"] = torch.nanmean(
            _stack_steps_with_tail("pf_sed_pca_dims", (PF_CRPS_PCA_DIMS,)),
            dim=0,
        )
    else:
        pf_sed_breakdown = _empty_pf_sed_breakdown(
    ...
    return {
        ...
        "res1_val": res1_val,
        "pf_sed_breakdown": pf_sed_breakdown,
    }
...
def _plot_pf_style_projections_from_cache(
    ...
    for bidx in plot_batch_indices:
        ...
        for step_label in plot_steps:
            ...
            prior_pca_mean, prior_pca_basis = _get_pf_cached_mode_pca_transform(
                pf_entry, mode="prior", range_idx=range_idx, bidx=b, max_dims=2
            )
            post_pca_mean, post_pca_basis = _get_pf_cached_mode_pca_transform(
                pf_entry, mode="post", range_idx=range_idx, bidx=b, max_dims=2
            )
            ...
            plot_and_test_point_clouds(
                ...
                prior_pca_source_tensor=prior_cloud.detach().cpu(),
                posterior_pca_source_tensor=post_cloud.detach().cpu(),
                prior_pca_mean=None if prior_pca_mean is None else prior_pca_mean.detach().cpu(),
                post_pca_mean=None if post_pca_mean is None else post_pca_mean.detach().cpu(),
                prior_pca_basis=None if prior_pca_basis is None else prior_pca_basis.detach().cpu(),
                post_pca_basis=None if post_pca_basis is None else post_pca_basis.detach().cpu(),
            )
...
def _build_ienks_model_args(args, forward_fun):
    ...
    Discrete maps pass a one-step map propagator and ignore rhs.
    """
    if args.dataset == "doubling1d":
        # Discrete one-step map: ignore rhs in iEnKS dispatcher.
        propagator = lambda func, u, t, dt: forward_fun(u)
        model_rhs = None
        steps_between_analyses = 1
        solve_with_pinv = True
    else:
        propagator = rk4
        model_rhs = forward_fun
    ...
    return {
        ...
        "solve_with_pinv": solve_with_pinv,
    }
...
def train_model(epoch, loader, model_list, optimizer, scheduler, args, H_info=None):
    """
    Train the model for one epoch with support for pre/post analysis losses.
    """
    model, infl_model, local_model, st_model1, st_model2 = model_list
    m = args.N
    losses = AverageMeter()
        ...
        with _autocast_context(args):
            ...
            if not args.running_loss and len(traj_cache['ens_f'] if need_pre else traj_cache['ens_a']) > args.ignore_first:
                ...
                for mode_cfg, key in [(pre_losses, 'ens_f'), (post_losses, 'ens_a')]:
                    ...
                    if mask.any():
                        t_loss = sum(l['weight'] * compute_loss(ens_tensor=ens_ts, batch_v=batch_v, loss_type=l['name'], ignore_first=0, end_ind=None, valid_B_mask=mask, norm_p=args.es_p, return_sum=True) for l in mode_cfg)
...
def generate_and_cache_pf_results(
    ...
    - For the ring dataset ('doubling1d'), it also saves no-history counterparts.
    ...
    def _compute_pf_distribution_summary(ens_tensor: torch.Tensor):
        ...
        # PCA summaries are stored in centered coordinates relative to the PF mean.
    ...
    batch_start_index = 0
    pf_ring_cdf_datasets = {"doubling1d"}
    ...
    with torch.no_grad():
        for batch_ind, batch_v in enumerate(loader):
            ...
            is_ring_dataset = str(getattr(args, "dataset", "")).lower() == "doubling1d"
...
def test_model(loader, model_list, args, infl=1, H_info=None, plot_figures=True, fig_name='example_fig', save_pdf=False):
    ...
    all_results = {
        ...
        'res1_step': None,
        'pf_sed': torch.empty(0, device=args.device),
        'pf_sed_state_mean': torch.empty(0, device=args.device),
        'pf_sed_pca_mean': torch.empty(0, device=args.device),
        'pf_sed_state_dims': torch.empty((0, PF_CRPS_STATE_DIMS), device=args.device),
        'pf_sed_pca_dims': torch.empty((0, PF_CRPS_PCA_DIMS), device=args.device),
    ...
    pf_style_projection_enabled = bool(
        ...
        and str(getattr(args, "dataset", "")).lower() != "doubling1d"
        and int(getattr(args, "ori_dim", 0)) >= 3
    )
    plot_prior_enabled = bool(
        plot_figures and (
            args.dataset in {"lorenz63", "doubling1d"} or pf_style_projection_enabled
        )
    )
    ...
    with torch.no_grad():
        for batch_ind, batch_v in enumerate(loader):
            ...
            for i in range(len(batch_v) - 1):
                ...
                if not active_mask.any():
                    ...
                    if use_running_metrics:
                        ...
                        pf_sed_step = None
                        if args.pf_verification:
                            pf_sed_step = _compute_dynamic_quantile_sed_from_pf_post_step(
            ...
            if use_running_metrics:
                ...
                pf_sed_breakdown = running_batch_metrics["pf_sed_breakdown"]
                ...
                if args.pf_verification:
                    pf_sed_breakdown = _compute_dynamic_quantile_sed_from_pf_post(
            ...
            all_results['rrmse_step'] = _append_metric_step_matrix(all_results['rrmse_step'], rrmse_step_vals)
            ...
            all_results['res1_step'] = _append_metric_step_matrix(all_results['res1_step'], res1_step_vals)
            _append_pf_sed_breakdown(all_results, pf_sed_breakdown)
    ...
    if all_results['rrmse'].numel() == 0:
        metrics_keys = ['mean_rmse', 'std_rmse',
                        'mean_rrmse', 'std_rrmse', 'mean_rrmse_step', 'std_rrmse_step',
                        'mean_rrmse_traj', 'std_rrmse_traj', 'mean_rmv', 'std_rmv',
                        ...
                        'mean_res1_step', 'std_res1_step', 'mean_res1_traj', 'std_res1_traj',
                        'mean_pf_sed', 'std_pf_sed',
                        'mean_pf_cov_diff', 'std_pf_cov_diff',
    ...
    # Keep PF-SED masking consistent with RMSE masking:
    # if valid_B_mask exists (even if all False), pass it through so summaries become NaN.
    valid_mask_for_pf = valid_B_mask if ('valid_B_mask' in locals()) else None
    _add_relative_metric_summary_metrics(final_metrics, all_results, valid_mask_for_pf, metric_key='rrmse')
    _add_relative_metric_summary_metrics(final_metrics, all_results, valid_mask_for_pf, metric_key='res1')
    _add_pf_sed_summary_metrics(final_metrics, all_results, valid_mask_for_pf)
...
def print_test_results(results):
    ...
    printed_rrmse_step = _print_mean_std_metric(results, "RRMSE-step", "mean_rrmse_step", "std_rrmse_step")
    printed_rrmse_traj = _print_mean_std_metric(results, "RRMSE-traj", "mean_rrmse_traj", "std_rrmse_traj")
    if not printed_rrmse_step and not printed_rrmse_traj:
        _print_mean_std_metric(results, "RRMSE", "mean_rrmse", "std_rrmse")
    elif not printed_rrmse_traj:
        _print_mean_std_metric(results, "RRMSE-traj", "mean_rrmse", "std_rrmse")
    ...
    printed_res1_step = _print_mean_std_metric(results, "RES1-step", "mean_res1_step", "std_res1_step")
    printed_res1_traj = _print_mean_std_metric(results, "RES1-traj", "mean_res1_traj", "std_res1_traj")
    if not printed_res1_step and not printed_res1_traj:
        _print_mean_std_metric(results, "RES1", "mean_res1", "std_res1")
    elif not printed_res1_traj:
        _print_mean_std_metric(results, "RES1-traj", "mean_res1", "std_res1")
    _print_mean_std_metric(results, "PF-SED", "mean_pf_sed", "std_pf_sed", skip_nan=True)
    pf_sed_state_dim_keys = [
        key
        for i in range(1, PF_CRPS_STATE_DIMS + 1)
        for key in (f'mean_pf_sed_state_dim{i}', f'std_pf_sed_state_dim{i}')
    ]
    pf_sed_pca_dim_keys = [
        key
        for i in range(1, PF_CRPS_PCA_DIMS + 1)
        for key in (f'mean_pf_sed_pca_dim{i}', f'std_pf_sed_pca_dim{i}')
    ]
    if _has_finite_metric_values(results, *pf_sed_state_dim_keys):
        state_parts = ", ".join(
            f"d{i}: {results[f'mean_pf_sed_state_dim{i}']:.3f} ± {results[f'std_pf_sed_state_dim{i}']:.3f}"
            for i in range(1, PF_CRPS_STATE_DIMS + 1)
        )
        print(f"PF-SED State Dims: {state_parts}")
    if _has_finite_metric_values(results, *pf_sed_pca_dim_keys):
        pca_parts = ", ".join(
            f"p{i}: {results[f'mean_pf_sed_pca_dim{i}']:.3f} ± {results[f'std_pf_sed_pca_dim{i}']:.3f}"
            for i in range(1, PF_CRPS_PCA_DIMS + 1)
        )
        print(f"PF-SED PCA Dims: {pca_parts}")
    if _has_finite_metric_values(
        ...
        *(
        ...
        print(
            "PF-SED Averages: "
            f"state={results['mean_pf_sed_state_avg']:.3f} ± {results['std_pf_sed_state_avg']:.3f}, "
            f"pca={results['mean_pf_sed_pca_avg']:.3f} ± {results['std_pf_sed_pca_avg']:.3f}, "
            f"total={results['mean_pf_sed']:.3f} ± {results['std_pf_sed']:.3f}"
        )
...
def test_ClassicFilter(loader, args, infl=1, H_info=None, plot_figures=True, fig_name='example_fig', loc_radius=None, save_pdf=False):
    ...
    if all_results['rrmse'].numel() == 0:
        metrics_keys = ['mean_rmse', 'std_rmse', 'mean_rrmse', 'std_rrmse',
                        'mean_rrmse_step', 'std_rrmse_step', 'mean_rrmse_traj', 'std_rrmse_traj',
    ...
    total_traj_count = int(all_results['rrmse'].numel())
    if 'valid_B_mask' in locals():
        valid_traj_count = int(torch.sum(valid_B_mask).item())
    else:
        valid_traj_count = 0
    final_metrics['total_traj_count'] = total_traj_count
    final_metrics['valid_traj_count'] = valid_traj_count
    final_metrics['valid_percent'] = final_metrics.get('no_nan_percent', float('nan'))
                    ...
                    if (i + 1) > args.ignore_first:
                        ...
                        if valid_B_mask_this_step.any():
                            step_loss = sum(compute_loss(
                                ...
                                return_sum=True) for lt in args.loss_type)
                    ...
                    if ens_tensor.shape[0] > 0:
                        ...
                        if num_valid_loss_contributions > 0:
                            ...
                            total_loss = sum(compute_loss(
                                ...
                                valid_B_mask=valid_B_mask, norm_p=args.es_p, return_sum=True,
...
def print_test_results_v2(results):
    ...
    printed_rrmse_step = _print_mean_std_metric(results, "RRMSE-step", "mean_rrmse_step", "std_rrmse_step", skip_nan=True)
    printed_rrmse_traj = _print_mean_std_metric(results, "RRMSE-traj", "mean_rrmse_traj", "std_rrmse_traj", skip_nan=True)
    if not printed_rrmse_step and not printed_rrmse_traj:
        _print_mean_std_metric(results, "RRMSE", "mean_rrmse", "std_rrmse", skip_nan=True)
    elif not printed_rrmse_traj:
        _print_mean_std_metric(results, "RRMSE-traj", "mean_rrmse", "std_rrmse", skip_nan=True)
    ...
    printed_res1_step = _print_mean_std_metric(results, "RES1-step", "mean_res1_step", "std_res1_step", skip_nan=True)
    printed_res1_traj = _print_mean_std_metric(results, "RES1-traj", "mean_res1_traj", "std_res1_traj", skip_nan=True)
    if not printed_res1_step and not printed_res1_traj:
