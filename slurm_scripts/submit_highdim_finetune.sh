...
# Input: List of
# (Dataset, Method, Checkpoint, Sigma, LossType, LearningRate, ObsFn, AdaptiveSigmaY[true/false], DetachSteps).
...
# List of experiments
# Format:
# "Dataset MethodName Checkpoint_Path Sigma_Y LossType LearningRate ObsFn AdaptiveSigmaY DetachSteps"
# EXPERIMENTS=(
...
for exp in "${EXPERIMENTS[@]}"; do
    ...
    echo "Submitting: $JOB_NAME (Dataset=$current_dataset, Sig=$current_sigma_y, Loss=$loss_type, Method=$v, LR=$current_lr, ObsFn=$current_obs_fn, Adaptive=$adaptive_sigma_y, DetachSteps=$detach_steps, NoLocalization=$no_localization)"

    # Submit job with exported variables
    sbatch -J "$JOB_NAME" \
           --time=10:00:00 \
           --gres="gpu:${GPU_TYPE}:1" \
           --export=ALL,DATASET=$current_dataset,SEED=$SEED,EPOCHS=$EPOCHS,SAVE_EPOCH=$SAVE_EPOCH,TRAIN_STEPS=$TRAIN_STEPS,TRAIN_TRAJ_NUM=$TRAIN_TRAJ_NUM,LR=$current_lr,ES_P=$ES_P,VERSION=$v,CP_PATH=$cp_path,SIGMA_Y=$current_sigma_y,LOSS_TYPE=$loss_type,OBS_FN=$current_obs_fn,ADAPTIVE_SIGMA_Y=$adaptive_sigma_y,DETACH_STEPS=$detach_steps,NORMAL_OUTPUT=$NORMAL_OUTPUT,NO_LOCALIZATION=$no_localization \
           $SLURM_SCRIPT
done
